...
const INITIAL_FIBONACCI_NUMBERS = {
	first: 0,
	second: 1
};

function productFib(targetProduct) {
	let currentFib = INITIAL_FIBONACCI_NUMBERS.first;
	let nextFib = INITIAL_FIBONACCI_NUMBERS.second;

	while (currentFib * nextFib < targetProduct) {
		const tempFib = nextFib;
		nextFib = currentFib + nextFib;
		currentFib = tempFib;
	}

	return [currentFib, nextFib, currentFib * nextFib === targetProduct];
}